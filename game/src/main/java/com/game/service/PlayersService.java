...
public class PlayersService {
    ...
    public Player createPlayer(Player player) {
        if (
            player.getName() == null || player.getTitle() == null || player.getRace() == null
    ...
    public Player getPlayer(String id) {
        ...
        if (!validId(newId))
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST);
        if (playersRepository.existsById(newId)) {
            return playersRepository.findById(newId).get();
        } else {
    ...
    public Player updatePlayer(Player playerNew, Player playerOld) {
        ...
        return playersRepository.save(playerOld);
    }
